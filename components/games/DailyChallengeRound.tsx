          ...
          <div className="mt-4 grid gap-2">
            {step.options.map((opt, i) => (
              <button
                ...
                disabled={revealed}
                onClick={() => checkMcq(i)}
                className="rounded-xl border border-[var(--line)] px-4 py-3 text-left text-sm text-[var(--foam)] hover:border-[var(--accent)] disabled:opacity-60"
              >
                {opt}
              </button>
            ))}
      ...
      {step.kind === "cloze" ? (
        <article className="rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-6">
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">
            {fmt(t.games.medium.hint, { hint: step.hint })}
          </p>
          <input
            value={textAnswer}
